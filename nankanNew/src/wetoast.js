...
function WeToastClass() {
  ...
  WeToast.prototype.hide = function () {
    ...
    setTimeout(() => {
      page.setData({
        __wetoast__: {},
        '__wetoast__.reveal': false // 2017/05/17 追加该行，不然点击以后不能用了 by yanbin
      })
    }, 400)
  }

  // 2017/05/17 显示加载图片 by yanbin
  WeToast.prototype.showLoading = function (data) {
    let page = this.__page
  ...
  // 2017/05/17 隐藏加载图片 by yanbin
  WeToast.prototype.hideLoading = function () {
    let page = this.__page
